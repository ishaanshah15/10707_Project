...
class Waterbirds(GroupDataset):
    def __init__(self, data_path, split, subsample_what=None, duplicates=None):
        ...
        metadata = os.path.join(data_path,"waterbird_complete95_forest2water2/metadata_waterbirds.csv")
...
class CivilComments(GroupDataset):
    def __init__(
        ...
        if 'subsamples' in data_path:
            metadata = os.path.join(data_path,"metadata_civilcomments_{}_stratified_subsample_005.csv".format(granularity))
            text = load_dataset(
                "csv", 
                data_files=os.path.join(data_path, "civilcomments_{}_stratified_subsample_005.csv".format(granularity)), 
                split='train'
            )
        else:
        ...
        self.tokenized_text = text.map(
            self.tokenize_function,
            batched=True,
            remove_columns=list(text.features.keys())
        )
        self.data_type = "text"
